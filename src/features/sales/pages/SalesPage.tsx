  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          {/* Desktop Table View */}
          <div id="sales-table-container" className="hidden md:block bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            {filteredRecords.map((rec) => (
              <div key={rec.id} className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm space-y-4 relative">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs border-t border-b border-slate-50 py-3">
                  <div>
                    <span className="font-bold text-slate-400 uppercase text-[9px] block tracking-wide">{t("buyer_name")}</span>
                    <span className="text-slate-700 font-semibold mt-1 block truncate">{rec.buyerName}</span>
                  </div>
                  <div>
